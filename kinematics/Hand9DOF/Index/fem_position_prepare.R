...
pos_abs_init <- pos_abs_init %>%
  ...
  separate_wider_delim(
    joint,
    delim = '_',
    names = c('joint', 'coord_segment', 'coord')
  ) %>%
  mutate(
...
pos_dat_inter_joint <- pos_dat %>%
  ...
  mutate(
    MCP_PIP_inter_dist = sqrt(
      (u1_PIP_start - u1_MCP_end)^2 + (u2_PIP_start - u2_MCP_end)^2
    ),
    PIP_DIP_inter_dist = sqrt(
      (u1_DIP_start - u1_PIP_end)^2 + (u2_DIP_start - u2_PIP_end)^2
    ),
    DIP_TIP_inter_dist = sqrt(
      (u1_TIP_start - u1_DIP_end)^2 + (u2_TIP_start - u2_DIP_end)^2
    )
  ) %>%
  ...
  summarise(
    across(
      everything(),
      list(max = max, min = min, range = ~ max(.x) - min(.x))
    )
  ) %>%
  mutate(
  ...
  pivot_longer(
    everything(),
    names_to = 'inter_joint_dist',
    values_to = 'value'
  ) %>%
  separate_wider_delim(
    inter_joint_dist,
    delim = '_',
    names = c('joint1', 'joint2', 'measure'),
    too_many = 'merge'
  ) %>%
...
joint_angles <- pos_dat %>%
  ...
  mutate(
    MCP_cum_angle = ifelse(
      MCP_cum_angle < -90,
      360 + MCP_cum_angle,
      MCP_cum_angle
    ),
    PIP_cum_angle = ifelse(
      PIP_cum_angle < -90,
      360 + PIP_cum_angle,
      PIP_cum_angle
    ),
    DIP_cum_angle = ifelse(
      DIP_cum_angle < -90,
      360 + DIP_cum_angle,
      DIP_cum_angle
    )
...
pos_bend_plt <- pos_dat %>%
  ...
  geom_path(
    # Downsample data to keep every 10th row after grouping per joint and coord_segment
    data = pos_dat %>%
  ...
  geom_text(
    data = pos_dat %>%
      ...
      filter(row_number() %% plt_point_dwns == plt_point_start) %>%
      filter(joint == 'TIP'),
    # Convert pressure to bar - round to 1 decimal
  ...
  # Label path at 0 bar with pressure at tip coordinates
  geom_text(
    data = pos_dat %>%
      group_by(joint, coord_segment) %>%
      filter(row_number() == 1) %>%
  ...
  geom_arc0(
    data = pos_dat_wide_arc %>%
      ...
      group_by(joint) %>%
      # filter(joint == 'MCP') %>%
      filter(row_number() %% plt_point_dwns == (plt_point_start - 1)) %>%
...
pos_dat_resample %>%
  ...
  ggplot(aes(
    x = pressure,
    y = coord_val,
    color = interaction(joint, coord_segment)
  )) +
...
pos_bend_resample_plt <- pos_dat_resample %>%
  ...
  geom_text(
    data = pos_dat_resample %>%
      ...
      filter(row_number() %% plt_point_dwns == plt_point_start) %>%
      filter(joint == 'TIP'),
    # Convert pressure to bar - round to 1 decimal place
    aes(label = round(pressure * 10, 1)),
    nudge_x = 4,
    nudge_y = -4,
    hjust = 0,
  ...
  # Label path at pressure = 0 with pressure at tip coordinates
  geom_text(
    data = pos_dat_resample %>%
      group_by(joint, coord_segment) %>%
      filter(row_number() == 1) %>%
...
pos_bend_model_compare_plt <- pos_dat_resample %>%
  ...
  mutate(
    L2_norm = sqrt(
      (max(resample_u1) - min(resample_u1))^2 +
        (max(resample_u2) - min(resample_u2))^2
    )
  ) %>%
  ungroup() %>%
  # summarise(
